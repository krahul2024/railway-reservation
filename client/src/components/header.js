...
const Header = () => {
	...
	return (<> 
		...
		<div>
			<div className="px-8 md:px-16 py-3 min-h-max shadow-2xl shadow-gray-800 items-center">
				<div className="flex gap-4 md:gap-12 justify-between m-2 md:mx-8 lg:mx-20 xl:mx-32">
					<NavLink to={"/"}
						className="flex gap-2 md:p-2 md:gap-4 items-center">
						...
						<span className="text-sky-500 font-bold text-lg">e-rail</span>
					</NavLink>

					<div className="flex gap-4 md:gap-8 md:p-2 items-center">
						
						<span className="text-sky-500 font-bold text-lg flex gap-2">
							{profile && (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
							  <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
							</svg> 
							)}
							<NavLink to={profile?"/user/profile":"/login"} className="text-sky-600 font-semibold text-lg hover:text-sky-400 cursor-pointer">{profile?profile.name.split(" ")[0]+"..":"Login"}</NavLink>
						</span>

						{profile === null && (
						<div>
							<NavLink to = {"/register"}
								className="p-1 m-1 text-sky-600 font-semibold text-lg hover:text-sky-400 cursor-pointer">
								Register</NavLink>
						</div>
						)}
						...
						<div>
							<span onClick = {(e) => processLogout(e) }
								className="p-1 m-1 text-sky-600 font-semibold text-lg hover:text-sky-400 cursor-pointer">logout</span>
						</div>
						)}
