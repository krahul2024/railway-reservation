...
const Login = () => {
	...
	const setLoginInputs = (e) => {
		const name = e.target.name , value = e.target.value 
		// console.log(name , value)
		setUser({...user,[name]:value})
	}

	const handleLoginInputs = async(e) => {
		e.preventDefault() 
		console.log(user) 
	...
	return (<> 
		...
		<div>
			<form action="" className="flex justify-center mt-28 md:mt-44 mb-40">
				...
				<div className="flex flex-col items-center px-4 py-12 w-auto rounded-xl backdrop-blur-xl">
					<div className="flex flex-col p-8 items-center rounded-lg ">
						<span className="p-1 font-extrabold text-3xl text-cyan-600"> Welcome Back!</span>
						<span className="italic text-sky-500">login to continue your ticket booking</span>
					</div>

					<div className="m-auto px-24 py-8 w-full flex flex-col items-center shadow-2xl backdrop-brightness-125 rounded-xl">
						<span className="flex py-3 text-4xl bg-transparent rounded-2xl shadow-2xl bg-sky-900 mb-16">
						...
						<div className="p-2 m-2 flex flex-col justify-between">
							<div className="flex flex-col p-2 mb-1">
								...
								<div className="flex mt-2">
										...
										<input type="text" value={user.username} name="username" onChange = { setLoginInputs}
											className="outline-0 bg-transparent px-8 border-b-2 border-cyan-700 text-indigo-500 hover:border-indigo-600 hover:brightness-150 shadow-2xl"
										/>
									</div>
							</div>
							...
							<div className="flex flex-col p-2">
								...
								<div className="flex">
									<div className="flex mt-2">
										...
										<input type={showPass} value={user.password} name="password" onChange = { setLoginInputs}
										className="outline-0 bg-transparent px-8 border-b-2 border-cyan-700 text-indigo-500 hover:border-indigo-600 hover:brightness-150 shadow-2xl"
										/>
									</div>
									
								<button className="px-4 mx-2 cursor-pointer text-cyan-500"
							...
							<div className="flex flex-col items-start mt-8">
								<button onClick = { (e) => handleLoginInputs(e)}
									...
									>Go</button>
								<span
									className="py-3 px-1 text-sky-500"
									>Not registered? <NavLink to = {"/register"}
										className="text-emerald-600 hover:font-semibold brightness-125"
										>Register</NavLink></span>
							</div>
						</div>
